...
export class User {
    ...
    if (this.watchlist) {
      if (this.watchlist.length > 0) {
        ...
        this.watchlist.map((item) => {
          ...
          if (item?.symbol) {
            temp.push(getFinancialAsset(item.symbol, item.type, item.tracker, item.price));
          }
        });
        return temp;
      }
  ...
  getItemFromWatchlist(symbol) {
    ...
    if (temp?.length > 0) {
      return getFinancialAsset(temp[0].symbol, temp[0].type, temp[0].tracker, temp[0].price);
    } else {
      return null;
    }
  }
