...
export const useNewsBySymbol = (symbol, limit = 70) => {
...
export const get24HoursChange = async (symbol) => {
  ...
  try {
    const res = await fetch(url.toString());
    const data = await res.json();
    console.log("debug get24HoursChange ", data["Global Quote"]);
    return data["Global Quote"];
  } catch (error) {
    console.log("debug get24HoursChange error", error);
  }
};
...
function formatDate(date) {
  ...
  return `${year}-${month}-${day} ${hours}:${paddedMinutes}:00`;
}
export const fetchCustomRangeData = async (symbol, tracker) => {
  ...
  try {
    ...
    const dataOnTrackerDate = timeSeries[formattedTrackerDate] || timeSeries[mostRecentDate];
    ...
    return { data: timeSeries, change, changeInPercentage, price };
  } catch (error) {
